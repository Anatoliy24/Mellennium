 ...
 * Time: 22:59
 */
$email = 'visa@mltravel.ru';
$subject = 'Форма обратной связи сайта MlGroup';
...
if (!empty($_REQUEST['email']) && filter_var($_REQUEST['email'],FILTER_VALIDATE_EMAIL) && !empty($_REQUEST['name']) && !empty($_REQUEST['message']))
{
  ...
  $body = "
    Была заполнена форма обратной связи на сайте MlGroup\r\n
    E-mail: ".$client['email']."\r\n
    Имя: ".$client['name']."\r\n
    Сообщение: ".$client['message']."\r\n
  ";
  $headers = array('From: MlGroup <no-reply@mltravel.ru>',
                   'MIME-Version: 1.0',
